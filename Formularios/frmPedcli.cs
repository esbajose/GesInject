...
{
    public partial class frmPedcli : Form
    {
        ...
        private void frmPedcli_Load(object sender, EventArgs e)
        {
        ...
        private void frmPedcli_KeyUp(object sender, KeyEventArgs e)
        {
            Control ctl;
            ctl = (Control)sender;
        ...
        private void sbrCargaPedidos()
        {
            ...
            try
            {
                bS1.DataSource = cPedidosVenta.Lista();
        ...
        private void sbrModifCampo(string vCampo, string vValor)
        {
            ...
            if (cabventa.aCampoModif != "") { vCampo = cabventa.aCampoModif; }
            if (cabventa.aValor != "") { vValor = cabventa.aValor; }
        ...
        private void sbrCargaLineas(string vPedido)
        {
            try
            {
                this.bS2.DataSource = cPedidosVenta.ListaLineas(vPedido);
        ...
        private void sbrCargaLinea(int vFil)
        {
            ...
            string vCanSer = grLinPed.Rows[vFil].Cells["CantidadServida"].Value.ToString();
            string vFechaEnt = grLinPed.Rows[vFil].Cells["FechaEntrega"].Value.ToString();
            string vLote = grLinPed.Rows[vFil].Cells["Lote"].Value.ToString();
            string vIva = grLinPed.Rows[vFil].Cells["IVA"].Value.ToString();
            string vCajas = grLinPed.Rows[vFil].Cells["Cajas"].Value.ToString();
            string vPrecio = grLinPed.Rows[vFil].Cells["Precio"].Value.ToString();
            string vDTO = grLinPed.Rows[vFil].Cells["DTO"].Value.ToString();
            string vDTOLin = grLinPed.Rows[vFil].Cells["DTOLin"].Value.ToString();
            ...
            txDTO.Text = vDTO;
            txDTOLin.Text = vDTOLin;

            btPrep.Visible = false;
            if (txPed.Text != "") txEnPrep.Text = cEntregas.fncEnPrep(txPed.Text, Convert.ToInt32(vLin)).ToString();
            if (txEnPrep.Text != "0") btPrep.Visible = true;
        ...
        private bool fncAltaLinea()
        {
            ...
            if (txCajas.Text == "") { txCajas.Text = "0"; }
            if (txCan.Text == "") { txCan.Text = "0"; }
            if (txDTO.Text == "") { txDTO.Text = "0"; }
            if (txDTOLin.Text == "") { txDTOLin.Text = "0"; }
        ...
        private void btGrabar_Click(object sender, EventArgs e)
        {
            if (lbSitu.Text == "Modificación")
            {
                ...
                if (vTx != vVal) { cLinVenta.fncGrabaCampo(vCT, vTx); }
        ...
        {
            string vTabla = "fpago";
            string vRes = cUtil.fncLista(vTabla, cParamXml.strOleDBConecDbf, "ccodpago", txFPago.Text, "", "", "", false, "", "", "", "DBF");
        ...
        private void btEntrega_Click(object sender, EventArgs e)
        {
            ...
            frmDirEntrega.vCliente = txCodCli.Text;
            frm.ShowDialog();

            int vIdEnt = frmDirEntrega.vIdEnt;
            if (vIdEnt != Convert.ToInt32(txIdEnt.Text))
            {
                this.SuspendLayout();
                txDirecciónEnt.Text = frmDirEntrega.vDirección;
                txPoblaciónEnt.Text = frmDirEntrega.vPoblación;
                txProvinciaEnt.Text = frmDirEntrega.vProvincia;
                txCPEnt.Text = frmDirEntrega.vCP;
                txIdEnt.Text = vIdEnt.ToString();

                sbrModifCampo("Ent_Dirección", frmDirEntrega.vDirección);
                sbrModifCampo("Ent_Población", frmDirEntrega.vPoblación);
                sbrModifCampo("Ent_CP", frmDirEntrega.vCP);
                sbrModifCampo("Ent_Provincia", frmDirEntrega.vProvincia);
                sbrModifCampo("Ent_Id", vIdEnt.ToString());
        ...
        {
            string vPrep= cEntregas.fncNumEnPrep(txPed.Text,Convert.ToInt32(txLinPed.Text));
            if (vPrep != "")
            {
